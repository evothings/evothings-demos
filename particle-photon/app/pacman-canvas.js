	...
	function Photon(){
		...
		this.disconnect = function () {
			...
			chrome.sockets.tcp.close(socketId, function() {
				...
				// Reset graphics and callbacks associated with the button in the main menu
				$('#' + previousDirection.name).css('background-color', '');

				$('#photon-control').text('Connect Photon');

				$(document).off('click','.button#photon-control').on('click','.button#photon-control',function(event) {
		    		photon.connect()
				});
			})
		};
	 	...
	 	function connectedCallback(result){
			...
			if (result === 0) {
				...
				// Update graphics and callbacks associated with the button in the main menu 
